...
func Update(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		idConvParaInt, err := strconv.Atoi(id)
		if err != nil {
			log.Println("Erro na conversão do Id para int", err)
		}

		precoConvParaFloar, err := strconv.ParseFloat(preco, 64)
		if err != nil {
			log.Println("Erro na conversão do preco para float64", err)
		}

		quantidadeConvParaInt, err := strconv.Atoi(quantidade)
		if err != nil {
			log.Println("Erro na conversão da quantidade para int", err)
		}

		models.AtualizaProduto(idConvParaInt, quantidadeConvParaInt, nome, descricao, precoConvParaFloar)
	}
	http.Redirect(w, r, "/", http.StatusMovedPermanently)
}
